...
# Modelo de Regresion del Ingreso para Clasificacion       #
...
train_hogares_s<- model.matrix(~ income + cabecera_resto + sexo + edad + edad_2 + nivel_educativo + tiempo_trabajando +
                                 tipo_de_trabajo + horas_trabajo + tam_emp, train_hogares_final) %>%
  as.data.frame()

# Ahora procedemos a dummyficar la base test_hogares_final

test_hogares_s<- model.matrix(~ cabecera_resto + sexo + edad + edad_2 + nivel_educativo + tiempo_trabajando +
...
fmla<-formula(income ~ cabecera_resto + poly(edad, 3):sexo + nivel_educativo + sexo:nivel_educativo + 
                poly(edad, 3):nivel_educativo + tiempo_trabajando +  tipo_de_trabajo + sexo:tipo_de_trabajo +
                poly(edad, 3):tipo_de_trabajo + horas_trabajo + horas_trabajo + sexo:horas_trabajo + tam_emp)
...
ridge<-train(fmla,
             ...
             tuneGrid = expand.grid(alpha = 0, #Ridge
                                    lambda = seq(0.01, 100000,by = 1000)),
             preProcess = c("center", "scale")
) 
...
test_hogares <- cbind(test_hogares, y_hat_lasso)

test_hogares <- test_hogares %>% mutate(ingreso = y_hat_lasso)
#test_hogares <- test_hogares %>% mutate(ingreso = exp(y_hat_EN))

test_hogares<- test_hogares %>% mutate(pobre1=ifelse(ingreso<Lp,1,0)) # Contrasto con linea de pobreza
...
ingresos_m5L <- cbind(y_hat_reg, y_hat_ridge, y_hat_lasso, y_hat_EN)
write.csv(ingresos_m5L, file="ingresos_m5L.csv",  row.names = F)
